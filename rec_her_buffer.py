...
from rec_dict_buffer import RecDictReplayBuffer, sample_obs_mem_queue
...
def cat_obs_queue(
    ...
    for key, space in observation_space.items():
        queue.append((cat_obs, obs1, obs2, key, space))

    while queue:
        parent_cat_obs, parent_obs1, parent_obs2, key, space = queue.pop(0)

        if isinstance(space, spaces.Dict):
            parent_cat_obs[key] = {}
            for subkey, subspace in space.items():
                queue.append((parent_cat_obs[key], parent_obs1[key], parent_obs2[key], subkey, subspace))
        else:
            parent_cat_obs[key] = th.cat((parent_obs1[key], parent_obs2[key]))
    return cat_obs

def norm_torch_obs_queue(
    obs: Dict,
    ...
    norm_torch_obs = {}
    for key, space in observation_space.items():
        queue.append((norm_torch_obs, obs, key, space))

    while queue:
        parent_nt_obs, parent_obs, key, space = queue.pop(0)

        if isinstance(space, spaces.Dict):
            parent_nt_obs[key] = {}
            for subkey, subspace in space.items():
                queue.append((parent_nt_obs[key], parent_obs[key], subkey, subspace))
        else:
            parent_nt_obs[key] = to_torch_func(norm_obs_func(parent_obs[key], env))
    return norm_torch_obs

class RecHerReplayBuffer(RecDictReplayBuffer):
    """
    Hindsight Experience Replay (HER) buffer for nested dictionary observation spaces.
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> DictReplayBufferSamples:
        ...
        observations = cat_obs_queue(real_data.observations, virtual_data.observations, self.observation_space)
        actions = th.cat((real_data.actions, virtual_data.actions))
        next_observations = cat_obs_queue(real_data.next_observations, virtual_data.next_observations, self.observation_space)
    ...
    def _get_virtual_samples(
        ...
        obs = sample_obs_mem_queue(self.observations,
                                   batch_indices,
                                   env_indices,
                                   self.observation_space,
                                   lambda x, y: x,
        ...
        next_obs = sample_obs_mem_queue(self.next_observations,
                                        batch_indices,
                                        env_indices,
                                        self.observation_space,
                                        lambda x, y: x,
        ...
        observations = norm_torch_obs_queue(obs,
                                            self.observation_space,
                                            self._normalize_obs,
                                            env,
                                            self.to_torch
                                            )
        next_observations = norm_torch_obs_queue(next_obs,
                                            self.observation_space,
                                            self._normalize_obs,
                                            env,
                                            self.to_torch
    ...
    def _sample_goals(self, batch_indices: np.ndarray, env_indices: np.ndarray) -> np.ndarray:
        ...
        return sample_obs_mem_queue(self.next_observations["achieved_goal"],
                                    transition_indices,
                                    env_indices,
